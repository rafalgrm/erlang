...
%% API
-export([start/1, init/1,multiplicate/1, divide/1]).
...
call_port(Msg) -> % wezwanie portu, wyslanie do procesu portowego podanej wiadomosci
  ...
  receive
    {problem, Result} -> io:format("~p",[Result]) % odebranie wyniku od procesu portowego
  end.
...
loop(Port) -> % port - nasluchuje wiadomosci i wywoluje odpowiednie wiadomosci
  receive
    {call, Caller, Msg} ->
      ...
      receive
        {Port, {data, Data}} ->
          Caller ! {problem, decode(Data)} % wyslanie zdekodowanego wyniku
      end,
      loop(Port)
  end.
